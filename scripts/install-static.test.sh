...
PATH="$dsh_fake_bin:$PATH" \
  ...
  scripts/install.sh >/dev/null
test -x "$dsh_install_dir/dsh-acp-activity"
...
grep -F "@deepseek-ai/dsh-llm-deepseek" scripts/install.ps1 >/dev/null
